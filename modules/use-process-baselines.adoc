...
[role="_abstract"]
In {product-title}, you can minimize your risks by using process baselining.
It is a proactive approach to keep your infrastructure secure.
{product-title} first discovers existing processes and creates a baseline.
...
* New processes (which are not in the process baseline) show up as risks but they do not trigger any violations.
...
* {product-title} stops adding processes to the process baselines.
* New processes (which are not in the process baseline) trigger violations.
