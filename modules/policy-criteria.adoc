...
* The *Regular expressions*, *AND, OR*, and *NOT* columns indicates whether you can use regular expressions and other logical operators along with the specific attribute.
`!` in the *Regular expressions* column indicates that you can only use regular expressions for the listed fields.
* You cannot use logical combination operators (`AND`, `OR`) for attributes which have:
** Boolean values (`true` and `false`)
** Minimum-value semantics, for example:
*** *Minimum RBAC permissions*
*** *Days since image was created*
* You cannot use the `NOT` logical operator for attributes which have:
** Boolean values (`true` and `false`)
...
The `Disallowed Image Label` policy criteria only works when you are using {product-title} version 3.0.40 or newer and link:/docs/integrate-with-other-tools/integrate-with-image-registries/[integrate with a Docker registry].
| ✓
| ✕
| ✓
| Deploy
...
You can use regular expressions (for both `key` and `value` fields) to match labels.
The `Required Image Label` policy criteria only works when you are using {product-title} version 3.0.40 or newer and link:/docs/integrate-with-other-tools/integrate-with-image-registries/[integrate with a Docker registry].
| ✓
| ✕
| ✓
| Deploy
...
| Containers running with the root filesystem configured as read only.
| ✕
| ✕
| ✕
| Deploy
...
| Check deployments for which process executions are not listed in the deployment's link:/docs/evaluate-security-risks/use-process-baselining/[locked process baseline].
| ✕
| ✕
| ✕
| Runtime
