...
** For *Excluded Images (Build Lifecycle only)* select all images from the list for which you do not want to trigger a violation for the policy.
+
[NOTE]
====
...
====
. In the *Policy Criteria* section, configure the attributes on which you want to trigger the policy.
//See the <<policy-criteria,Policy criteria>> section for more details.
+
[NOTE]
====
If you are using {product-title} version 3.0.45 or newer:

* Select *Next* to view the *Policy Criteria* section.
====
