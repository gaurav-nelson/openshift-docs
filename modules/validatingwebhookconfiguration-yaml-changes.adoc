...
* Beginning for {product-title} version 3.0.55, you can enforce security policies on pod execution and pod port forward events.
...
To increase reliability, you could increase the replicas by running the following command:
* On {ocp}:
+
[source,terminal]
----
...
[source,terminal]
----
$ kubectl -n stackrox scale deploy/admission-control --replicas=<number_of_replicas>
----
